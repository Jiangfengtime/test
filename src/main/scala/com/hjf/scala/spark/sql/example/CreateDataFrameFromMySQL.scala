package com.hjf.scala.spark.sql.example
...
object CreateDataFrameFromMySQL {
  def main(args: Array[String]): Unit = {
    val spark = SparkSession.builder().master("local").appName("createdataframefrommysql")
      .config("spark.sql.shuffle.partitions",1) //默认分区数为200
      .getOrCreate()
    ...
    val person: DataFrame = spark.read.jdbc("jdbc:mysql://node01/spark","person",properties)
    person.show()
    ...
    val map = Map[String,String](
      "url"->"jdbc:mysql://node01:3306/spark",
    ...
    val reader: DataFrameReader = spark.read.format("jdbc")
      .option("url", "jdbc:mysql://node01:3306/spark")
    ...
    result.write.mode(SaveMode.Append).jdbc("jdbc:mysql://node01/spark", "result", properties)

//    /**
//      * 读取mysql中数据的第四种方式
//      */
    spark.read.jdbc("jdbc:mysql://node01/spark","(select person.id,person.name,person.age,score.score from person ," +
      "score where  person.id = score.id) T",properties).show()
